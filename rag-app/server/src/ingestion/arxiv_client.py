...
def fetch_papers_paginated(query: str, max_results: int = 20, 
    results_per_page: int = 5, wait_time: int = 5, save_local = True):
    start = 0
    papers = []
    for i in range(start, max_results, results_per_page):
       ...
       if save_local:
           with open(f'''{DATA_PATH}/papers_{i}_{i+results_per_page}.json''', "w") as f:
